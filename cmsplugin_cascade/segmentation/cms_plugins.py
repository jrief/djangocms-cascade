...
class SegmentFormMixin(EntangledModelFormMixin):
    ...
    def clean(self):
        ...
        open_tag, condition = cleaned_data['glossary']['open_tag'], cleaned_data['glossary']['condition']
        if open_tag in ('if', 'elif'):
            if not condition:
                raise ValidationError(_("The evaluation condition is missing or empty."))
            try:
                condition = unescape_entities(condition)
            ...
            cleaned_data['glossary']['condition'] = ''  # empty condition for else-block
        return cleaned_data
