...
class BootstrapImagePlugin(ImageAnnotationMixin, LinkPluginBase):
    ...
    def render(selcompute_aspect_ratiof, context, instance, placeholder):
        tags = utils.get_image_tags(context, instance)
    ...
    def sanitize_model(cls, obj):
        column = obj.get_ancestors().filter(plugin_type='BootstrapColumnPlugin').order_by('depth').last()
        min_max_bounds = column.get_bound_plugin().get_grid_instance().get_min_max_bounds()
        if obj.glossary.get('bounds') != min_max_bounds:
