...
class BootstrapException(Exception):
    """
    Raised if arrangement of Bootstrap-5 elements would render weird results.
    """
...
class Bootstrap5Container(list):
    """
    Abstracts a Bootstrap-5 container element, such as ``<div class="container">...</div>``, so that the minimum and
    maximum widths each each child row can be computed.
    Each container object is a list of one to many ``Bootstrap5Row`` instances.
    In order to model a "fluid" container, use ``fluid_bounds`` during construction.
    """
    # def __init__(self, bounds=app_settings.CMSPLUGIN_CASCADE['bootstrap5']['default_bounds']):
    ...
    def add_row(self, row):
        if isinstance(row.parent, (Bootstrap5Container, Bootstrap5Column)):
...
class Bootstrap5Row(list):
    """
    Abstracts a Bootstrap-5 row element, such as ``<div class="row">...</div>``, so that the minimum and maximum widths
    each each child columns can be computed.
    Each row object is a list of 1 to many ``Bootstrap5Column`` instances.
    """
    parent = None
    bounds = None
    ...
    def add_column(self, column):
        if isinstance(column.parent, Bootstrap5Row):
...
class Bootstrap5Column(list):
    """
    Abstracts a Bootstrap-5 column element, such as ``<div class="col...">...</div>``, which shall be added to a
    ``Bootstrap5Row`` element.
    Each column may contain elements of type ``Bootstrap5Row``.
